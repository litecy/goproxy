...
import (
	...
	"errors"
	"fmt"
	"github.com/litecy/goproxy/pkg/core/dst"
	kcpcfg2 "github.com/litecy/goproxy/pkg/core/lib/kcpcfg"
	"github.com/litecy/goproxy/pkg/core/lib/transport"
	"github.com/litecy/goproxy/pkg/core/lib/transport/encrypt"
...
func (s *ServerChannel) ListenTCPS(method, password string, compress bool, fn func(conn net.Conn)) (err error) {
	_, err = encrypt.NewCipher(method, password)
	if err != nil {
		return
	}
	return s.ListenTCP(func(c net.Conn) {
		if compress {
			c = transport.NewCompConn(c)
		}
		c, _ = encrypt.NewConn(c, method, password)
		fn(c)
	})
}
...
func (s *ServerChannel) ListenKCP(config kcpcfg2.KCPConfigArgs, fn func(conn net.Conn), log *logger.Logger) (err error) {
				...
				if err == nil {
					go func() {
						...
						if *config.NoComp {
							...
							cconn := transport.NewCompStream(conn)
							fn(cconn)
						}
					}()
				} else {
...
func (s *ServerChannel) ListenTOU(method, password string, compress bool, fn func(conn net.Conn)) (err error) {
	...
	s.TOUServer = dst.NewMux(s.UDPListener, 0)
	go func() {
		defer func() {
			if e := recover(); e != nil {
		...
		for {
			...
			if err == nil {
				go func() {
					...
					if compress {
						conn = transport.NewCompConn(conn)
					}
					conn, err = encrypt.NewConn(conn, method, password)
					if err != nil {
						conn.Close()
						s.log.Println(err)
						return
