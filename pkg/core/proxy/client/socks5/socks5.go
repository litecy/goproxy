...
type Dialer struct {
	...
	usernamePassword *socks5.UsernamePassword
}

// NewDialer returns a new Dialer that dials through the provided
// proxy server's network and address.
func NewDialer(auth *socks5.UsernamePassword, timeout time.Duration) *Dialer {
...
// network : tcp udp
func NewClientConn(conn *net.Conn, network, target string, timeout time.Duration, auth *socks5.UsernamePassword, header []byte) *ClientConn {
	s := &ClientConn{
		conn:    conn,
		network: network,
		timeout: timeout,
...
func (s *ClientConn) _Handshake() error {
	...
	buf := []byte{}
	if s.network == "tcp" {
		buf = append(buf, socks5.VERSION_V5, socks5.CMD_CONNECT, 0 /* reserved */)

	} else {
		buf = append(buf, socks5.VERSION_V5, socks5.CMD_ASSOCIATE, 0 /* reserved */)
	}
	if ip := net.ParseIP(host); ip != nil {
		if ip4 := ip.To4(); ip4 != nil {
			buf = append(buf, socks5.ATYP_IPV4)
			ip = ip4
		} else {
			buf = append(buf, socks5.ATYP_IPV6)
		}
		buf = append(buf, ip...)
	} else {
		if len(host) > 255 {
		...
		buf = append(buf, socks5.ATYP_DOMAIN)
	...
	if int(buf[1]) < len(socks5.Socks5Errors) {
		failure = socks5.Socks5Errors[buf[1]]
	}
...
func (s *ClientConn) auth(host string) error {
	...
	buf = append(buf, socks5.VERSION_V5)
	if len(s.user) > 0 && len(s.user) < 256 && len(s.password) < 256 {
		buf = append(buf, 2 /* num auth methods */, socks5.Method_NO_AUTH, socks5.Method_USER_PASS)
	} else {
		buf = append(buf, 1 /* num auth methods */, socks5.Method_NO_AUTH)
	}
	(*s.conn).SetDeadline(time.Now().Add(s.timeout))
	if _, err := (*s.conn).Write(buf); err != nil {
	...
	// See RFC 1929
	if buf[1] == socks5.Method_USER_PASS {
		buf = buf[:0]
