...
import (
	...
	"errors"
	"fmt"
	dnsx2 "github.com/litecy/goproxy/pkg/utils/dnsx"
	mapx2 "github.com/litecy/goproxy/pkg/utils/mapx"
	sni2 "github.com/litecy/goproxy/pkg/utils/sni"
	"io"
	"io/ioutil"
	logger "log"
	"net"
...
type Checker struct {
	data        mapx2.ConcurrentMap
	blockedMap  mapx2.ConcurrentMap
	directMap   mapx2.ConcurrentMap
	interval    int64
	timeout     int
	isStop      bool
	intelligent string
...
func (c *Checker) loadMap(f string) (dataMap mapx2.ConcurrentMap) {
	dataMap = mapx2.NewConcurrentMap()
	if PathExists(f) {
...
type BasicAuth struct {
	...
	authTimeout int
	authRetry   int
	dns         *dnsx2.DomainResolver
	log         *logger.Logger
}

func NewBasicAuth(dns *dnsx2.DomainResolver, log *logger.Logger) BasicAuth {
	return BasicAuth{
		data: mapx2.NewConcurrentMap(),
		dns:  dns,
		log:  log,
...
func NewHTTPRequest(inConn *net.Conn, bufSize int, isBasicAuth bool, basicAuth *BasicAuth, log *logger.Logger, header ...[]byte) (req HTTPRequest, err error) {
	...
	//try sni
	serverName, err0 := sni2.ServerNameFromBytes(req.HeadBuf)
	if err0 == nil {
		//sni success
		req.Method = "SNI"
...
func (cm *ConnManager) Add(key, ID string, conn *net.Conn) {
	cm.pool.Upsert(key, nil, func(exist bool, valueInMap interface{}, newValue interface{}) interface{} {
		var conns mapx2.ConcurrentMap
		if !exist {
			conns = mapx2.NewConcurrentMap()
		} else {
			conns = valueInMap.(mapx2.ConcurrentMap)
		}
		if conns.Has(ID) {
			v, _ := conns.Get(ID)
			(*v.(*net.Conn)).Close()
...
type ClientKeyRouter struct {
	keyChan chan string
	ctrl    *mapx2.ConcurrentMap
	lock    *sync.Mutex
}

func NewClientKeyRouter(ctrl *mapx2.ConcurrentMap, size int) ClientKeyRouter {
