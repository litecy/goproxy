...
type Selector interface {
	...
	Stop()
	Reset(configs BackendsConfig, dr *dnsx2.DomainResolver, log *log.Logger)
	IsActive() bool
...
func NewGroup(selectType int, configs BackendsConfig, dr *dnsx2.DomainResolver, log *log.Logger, debug bool) Group {
	bks := []*Backend{}
...
func (r *RoundRobin) Reset(configs BackendsConfig, dr *dnsx2.DomainResolver, log *log.Logger) {
	r.Lock()
	defer r.Unlock()
	r.Stop()
	bks := []*Backend{}
...
func (lc *LeastConn) Reset(configs BackendsConfig, dr *dnsx2.DomainResolver, log *log.Logger) {
	lc.Lock()
	defer lc.Unlock()
	lc.Stop()
	bks := []*Backend{}
...
func (h *Hash) Reset(configs BackendsConfig, dr *dnsx2.DomainResolver, log *log.Logger) {
	h.Lock()
	defer h.Unlock()
	h.Stop()
	bks := []*Backend{}
...
func (w *Weight) Reset(configs BackendsConfig, dr *dnsx2.DomainResolver, log *log.Logger) {
	w.Lock()
	defer w.Unlock()
	w.Stop()
	bks := []*Backend{}
...
func (lt *LeastTime) Reset(configs BackendsConfig, dr *dnsx2.DomainResolver, log *log.Logger) {
	lt.Lock()
	defer lt.Unlock()
	lt.Stop()
	bks := []*Backend{}
